...
const translateEnToTe = async (text) => {
  ...
  try {
    const parts = await Promise.all(chunks.map(async (chunk) => {
      const url = `https://translate.googleapis.com/translate_a/single?client=gtx&sl=auto&tl=te&dt=t&q=${encodeURIComponent(chunk)}`
...
export const fetchVerseTafsir = async (surahNumber, verseNumber) => {
  ...
  const promise = (async () => {
    ...
    return null
  })().finally(() => { delete pendingTafsirRequests[key] })

  pendingTafsirRequests[key] = promise
  return promise
}
